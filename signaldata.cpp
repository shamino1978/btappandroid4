...
int SignalData3::size() const
{
    return d_data->values.size();
}

QPointF SignalData3::value( int index ) const
{
    return d_data->values[index];
}

QRectF SignalData3::boundingRect() const
{
    return d_data->boundingRect;
}

void SignalData3::lock()
{
    d_data->lock.lockForRead();
}

void SignalData3::unlock()
{
    d_data->lock.unlock();
}

void SignalData3::append( const QPointF &sample )
{
    d_data->mutex.lock();
    d_data->pendingValues += sample;
...
void SignalData3::clearStaleValues( double limit )
{
    d_data->lock.lockForWrite();
...
SignalData3 &SignalData3::instance()
{
    static SignalData3 valueVector;
    return valueVector;
}
